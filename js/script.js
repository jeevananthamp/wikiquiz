...
function load_question() {
  ...
  if (q_idx === 10) {
    /* if we've loaded all 10 questions, we need more data */
    q_idx = 0;
    ...
    var redacted = convert_to_redacted(text, correct[1], label);
...
function getNearbyLocations(correct, lat, lon) {
  ...
  xmlHttp.open(
    "GET",
    "http://getnearbycities.geobytes.com/GetNearbyCities?callback=?&radius=100&latitude=" +
      lat +
      "&longitude=" +
      lon,
    true
...
function getCoords(query) {
  ...
  xmlHttp.onreadystatechange = function() {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
      received_coords(
        query,
        JSON.parse(xmlHttp.responseText)["results"][0]["geometry"]["location"]
      );
    }
  };
  xmlHttp.open(
    ...
    "https://maps.googleapis.com/maps/api/geocode/json?address=" +
      query.replace(" ", "+") +
      "&key=AIzaSyD8U9Br6Pak76wmlPpO2nVsEjgaMLUzPgs",
    true
  );
  xmlHttp.send(null);
}
...
function displayQuestion(correct, wrong_answers) {
  ...
  if (Array.isArray(correct)) {
    correct_answer_modified =
      correct[2]; /* use the modified answer for the multiple choice */
  } else {
...
function get_wrong_answers(label, correct_answer, other_gaps, gap_index) {
  ...
  var wrong_answer_1 = get_wrong_answer(
    label,
    correct_answer,
    other_gaps,
    gap_index
  ).toString();
  var wrong_answer_2 = get_wrong_answer(
    label,
    correct_answer,
    other_gaps,
    gap_index
  ...
  for (var i = 0; i < max_num_retries; i++) {
    ...
    wrong_answer_1 = get_wrong_answer(
      label,
      correct_answer,
      other_gaps,
      gap_index
    ).toString();
    wrong_answer_2 = get_wrong_answer(
      label,
      correct_answer,
      other_gaps,
      gap_index
  ...
  if (any_answers_same) {
    ...
    to_ret = [
      (parseInt(wrong_answer_1) + 1).toString(),
      (parseInt(wrong_answer_2) - 1).toString()
    ];
  }
  ...
  if (label === "PROPER") {
    ...
    if (num.toString() === num_as_str) {
      ...
      if (mightBeYear(num)) {
        ...
        if (num <= 2017) {
          return Math.min(generated, 2017);
        }

        return generated;
      }
      to_ret = parseInt(
        Math.max(0, num + getRandomInt(num * 0.01, num * 2) * posOrNeg())
      );
      if (to_ret > 30 && to_ret < 45) {
        to_ret = 31;
      }
      return to_ret;
    } else {
      return (
        parseFloat(num_as_str) + parseFloat(num_as_str) * 0.01 * posOrNeg()
      );
    }
  }
}
